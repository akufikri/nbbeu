...
(() => {
  ...
  var _ = (t) => {
    let e = (r, i, n) => {
      ...
      A(d)
        ? (u = d)
        : (u = d(n && "instanceIndex" in n ? n.instanceIndex : void 0));
  ...
  var L = ({ scriptAttributes: t, attributeKey: e, version: o, init: s }) => {
      var c;
      Z(), (c = window.fsAttributes)[e] || (c[e] = {});
      let { preventsLoad: r, attributes: i } = P(t),
    ...
    tt = () => {
      let t = S("src", "finsweet", { operator: "contains" }),
        e = S("dev");
      return [...document.querySelectorAll(`script${t}, script${e}`)].reduce(
        (r, i) => {
          var c;
          let n =
            i.getAttribute(f.dev.key) ||
  ...
  var Y = (t, e, o) => {
    ...
    t.hasAttribute(g) || t.setAttribute(g, s),
      t.hasAttribute(U) || t.setAttribute(U, s);
  };
  var F = (t) => {
      let { textContent: e } = t;
      if (!e) return null;
  ...
  var H = (t, e, o) => {
    let s = (c) => {
      ...
      r = k(async () => {
        let { top: c } = t.getBoundingClientRect();
        s(c);
      }, 100),
      i = new IntersectionObserver((c) => {
        for (let { isIntersecting: a, intersectionRect: l } of c)
          window[a ? "addEventListener" : "removeEventListener"]("scroll", r),
            a && s(l.top);
      }),
      n = () => {
  ...
  var E = `fs-${T}`,
    ...
    at = "locale",
    v = {
      element: { key: `${E}-element`, values: { number: nt } },
      start: { key: `${E}-${st}` },
      end: { key: `${E}-${it}` },
      duration: { key: `${E}-${ct}` },
      threshold: { key: `${E}-${ut}` },
      locale: { key: `${E}-${at}`, values: { auto: "auto" } },
    },
    [te, j, b] = _(v),
    q = 0,
    G = 1e3,
  ...
  var W = async () => {
    await C(B);
    let t = j("number", { operator: "prefixed", all: !0 }),
      e = t.map(z).filter(y);
    return h(T, t, () => {
      for (let o of e) o();
    });
  };
  L({ init: W, version: M, attributeKey: T });
})();
